...
def travelling_salesman(points, start=None):
    ...
    if start is None:
        start = points[0]
    var = min([perm for perm in permutations(points) if perm[0] == start], key=total_distance)
    print("\n")
    print("Path followed by travelling salesman is " + str(var))
    return var
...
def optimized_travelling_salesman(points, start=None):
    ...
    print("\n")
    print("Path followed by optimised travelling salesman is " + str(path))
    return path
...
def main():
    ...
    points = [[0, 0], [1, 5.7], [2, 3], [3, 7],
              [0.5, 9], [3, 5], [9, 1], [10, 5]]
    print("""Input order: {}.""".format(tuple(points)))
    print("""
Starting point is {} \n
Minimum distance using travelling salesman is {}. But it takes N! combinations. \n
The optimized algorithm yields a longer path but saving time and memory {}.""".format(
        points[0],
